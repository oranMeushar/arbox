...
const Building = () => {
    ...
    useEffect(() => {
        const buttons = [];
        if(Object.values(building).length) {
            Object.values(building).forEach((floor, idx) => {
                buttons.push({floorIndex: floor.floorIndex, button: React.createRef()});
            });
        }
        setButtonsRef(buttons);
    }, [building]);
            ...
            <Structure>
                ...
                }
                {
                    positions.map((position, idx) => {
                        return <ImageContainer horizontalPosition={`${position.horizontalPosition}%`} verticalPosition={`${position.verticalPosition}%`}>
                            <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 50 50' fill='#000000'>
                                <path 
                                    fill={(elevators[idx].isMoving && !elevators[idx].hasReachedFloor ) ? 'red' : (elevators[idx].isMoving && elevators[idx].hasReachedFloor ) ? 'green' : 'black'}
